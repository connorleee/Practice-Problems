...
function findRequiredFuel(modulesArr) {
    // Running total of required fuel
    let requiredFuel = 0;
    
    // loop through and perform conversion to fuel and add to the required fuel
    for (let i = 0; i < modulesArr.length; i++) {
        const currentModule = modulesArr[i];
        
        let fuelRequiredForModule = Math.floor(currentModule/3) -2;

        requiredFuel = requiredFuel + fuelRequiredForModule;
    }

    return requiredFuel;
}

console.log(findRequiredFuel(modules));