...
function nextLocation(currentLocation, command) {
    ...
    switch (direction) {
        ...
        case "L":
            ...
            break;
        default:
            throw new Error("Invalid command. Please use U, R, D, or L");
    }

    return currentLocation
}

// finds distance from [0,0]
function getDistance(location) {
    return Math.abs(location[0]) + Math.abs(location[1]);
}

function storeLocations(commandArr) {
    let location = [0,0];

    for (let i = 0; i < commandArr.length; i++) {
        const currentCommand = commandArr[i];

        // overwrites location variable with the new current location 
        const nextLoc = nextLocation(location, currentCommand);
        const distance = getDistance(nextLoc);
...
// console.log(nextLocation(testLocation, w1[0]))
// console.log(nextLocation(testLocation, w1[1]))
...
console.log(findClosestIntersection(sortedLocationsVisited))
