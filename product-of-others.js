function getProductsOfAllIntsExceptAtIndex(intArray) {
    if (intArray.length < 2) {
        throw new Error("can't compute product");
    }

    // Make a list of the products
    let prodOfAllIntsExceptAtIndex = [];

    // pushing in the product of all before each index
    let productSoFar = 1;
    for (let i = 0; i < intArray.length; i++) {
        const currentInt = intArray[i];
        prodOfAllIntsExceptAtIndex[i] = productSoFar;
        productSoFar *= currentInt;
    }

    // pushing in the product of all after each index
    productSoFar = 1;
    for (let j = intArray.length - 1; j >= 0; j--) {
        prodOfAllIntsExceptAtIndex[j] *= productSoFar;
        productSoFar *= intArray[j];
    }

    return prodOfAllIntsExceptAtIndex;
}
